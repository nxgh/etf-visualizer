...
export async function fetchDanJuan<T = Response>(
  ...
  return (parseJSON ? fetchJSON<T>("danjuan", url, fetchOptions) : fetchWithPlatform("danjuan", url, fetchOptions)) as Promise<
    T extends Response ? Response : T
  >;
}
...
export async function fetchXueQiu<T = Response>(
  ...
  const { parseJSON = true, ...fetchOptions } = options || {};
  return (parseJSON ? fetchJSON<T>("xueqiu", url, fetchOptions) : fetchWithPlatform("xueqiu", url, fetchOptions)) as Promise<
    T extends Response ? Response : T
  >;
}
...
export async function fetchWeibo<T = Response>(
  ...
  const { parseJSON = true, ...fetchOptions } = options || {};
  return (parseJSON ? fetchJSON<T>("weibo", url, fetchOptions) : fetchWithPlatform("weibo", url, fetchOptions)) as Promise<
    T extends Response ? Response : T
  >;
}
