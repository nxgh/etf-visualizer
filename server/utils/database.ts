import type { BlogParsed } from "#server/site/weibo/type";
...
export const insertPost = async (list: BlogParsed[]) => {
  try {
    ...
    console.info("插入帖子成功", { result });
    return true;
  } catch (error) {
    console.error("插入帖子失败", { error, values: list.map((i) => i.id) });
    throw error;
  }
};
...
export const findLatestPostByUserId = async (userId: string) => {
  try {
    const [rows] = await pool.execute<mysql.RowDataPacket[]>("SELECT * FROM weibo_posts WHERE user_id = ? ORDER BY time DESC LIMIT 1", [
      userId,
    ]);
    return rows[0] as BlogParsed;
  } catch (error) {
    console.error("查询最新博文失败", { error });
    return null;
  }
};
...
export const updateUserFirstBlogId = async (userId: string | number, firstBlogId: string) => {
  try {
    const [result] = await pool.execute("UPDATE weibo_users SET first_blog_id = ? WHERE user_id = ? AND first_blog_id IS NULL", [
      firstBlogId,
      userId,
    ]);
    console.info("更新用户第一条博文ID成功", { result });
    return true;
  } catch (error) {
    console.error("更新用户第一条博文ID失败", { error });
    throw error;
  }
};
...
export const checkPostExists = async (blogIds: string[]): Promise<string[]> => {
  try {
    ...
    console.error("检查博文是否存在失败", { error });
    throw error;
  }
};

export const findSecurityHistoryBySymbol = async (symbol: string): Promise<{ columns: string[]; rows: (number | string)[][] } | null> => {
  try {
    const [rows] = await pool.execute<mysql.RowDataPacket[]>("SELECT * FROM security_history WHERE symbol = ?", [symbol]);
    return {
      columns: Object.keys(rows[0]),
      rows: rows.map((row) => Object.values(row)),
    };
  } catch (error) {
    console.error("查询证券历史数据失败", { error, symbol });
    return null;
  }
};
...
export const findSecurityBySymbol = async (symbol: string): Promise<SecurityInfo | null> => {
  try {
    ...
    console.error("查询证券信息失败", { error, symbol });
    return null;
  }
};
...
export const insertOrUpdateSecurity = async (security: SecurityInfo) => {
  try {
    ...
    console.info("插入或更新证券信息成功", { symbol: security.symbol });
    return true;
  } catch (error) {
    console.error("插入或更新证券信息失败", { error, symbol: security.symbol });
    throw error;
  }
};
...
export const insertOrUpdateSecurityHistory = async (
  ...
  try {
    ...
    console.info("插入或更新证券历史数据成功", { symbol, length: securityHistory.length });
    return true;
  } catch (error) {
    console.error("插入或更新证券历史数据失败", { error, symbol });
    throw error;
  }
};
...
export const findSecurityHistoryBySymbolAndDate = async (symbol: string, begin: string, end: string) => {
  try {
    ...
    console.error("查询证券历史数据失败", { error, symbol, begin, end });
    return null;
  }
};
