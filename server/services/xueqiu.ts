import type { ChartKlineJSON } from "../site/xueqiu/types";
import {
...
import * as XueQiu from "../site/xueqiu/index";
...
export async function fetchStockDetailAndKline(symbol: string) {
  ...
  try {
    ...
    console.error("获取股票详情和K线失败", { error });
    throw error;
  }
}
...
class XueQiuService {
  ...
  async fetchStockDetailAndKline(symbol: string) {
    try {
      ...
      console.error("获取详情和K线失败", { error });
      return null;
    }
  }
  async batchQuote(symbols: string) {
    try {
      const res = await XueQiu.batchQuote(symbols);
      return res;
    } catch (error) {
