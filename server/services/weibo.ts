import * as Weibo from "../site/weibo/index";
import type { BlogJSONType } from "../site/weibo/type.d";
import type { BlogParsed } from "../site/weibo/type.d";
import { findUserById, insertUser, updateUserFirstBlogId, insertPost, findLatestPostByUserId, type WeiboUser } from "../utils/database";
...
export async function parseBlogList(list: BlogJSONType["data"]["list"]): Promise<BlogParsed[]> {
  try {
    ...
    for (const item of list) {
      const ref_link = item?.retweeted_status ? `${item?.retweeted_status?.user?.id}/${item?.retweeted_status?.mblogid}` : "";
